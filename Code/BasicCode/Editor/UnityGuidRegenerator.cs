...
namespace UnityGuidRegenerator_wjy
{
    public class UnityGuidRegeneratorMenu
    {
        [MenuItem("Tools_wjy/随机生成新GUID")]
        public static void RegenerateGuids()
        {
            if (EditorUtility.DisplayDialog("GUID生成",
                "即将随机生成新的GUID，可能会造成部分未知的错误。 \n\n 为防意外请备份程序。",
                "选择随机生成GUID文件所属文件夹", "取消"))
            {
                try
                {
                    AssetDatabase.StartAssetEditing();

                    string path = EditorUtility.OpenFolderPanel("选择随机生成GUID文件所属文件夹", "Assets", "");
                    path.Replace('/', '\\');
